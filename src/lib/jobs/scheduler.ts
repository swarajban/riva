...
export async function scheduleRequestExpiration(schedulingRequestId: string): Promise<void> {
  const expiresAt = new Date(Date.now() + config.timing.requestExpirationMs);

  await db.update(schedulingRequests).set({ expiresAt }).where(eq(schedulingRequests.id, schedulingRequestId));
}
